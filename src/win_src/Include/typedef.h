...
/* __max/__min - макросы старого MSVC CRT (<stdlib.h> под _WIN32), их
   нет в стандартной библиотеке g++/Linux. Семантика идентична обычному
   тернарному сравнению ((a)>(b)?(a):(b)), поэтому нейтрализуем здесь
   как совместимость компилятора, без изменения вычислений. */
#ifndef __max
...
// typedef _real _far * r_array;
 ...
 // typedef _complex _far * c_array;
  ...
  /* Под MSVC структура _complex (поля .x/.y) бралась из системного
     CRT-заголовка <complex.h> (legacy _complex API), который компилятор
     подключал неявно. g++/Linux такого заголовка не имеет, поэтому тип
     объявляется здесь явно - идентично C-ветке выше (__cplusplus не задан)
     и реальному использованию в коде (поля .x/.y), без изменения состава
     или семантики типа. */
  #ifndef __COMPLEX_DEFINED
    #define __COMPLEX_DEFINED
