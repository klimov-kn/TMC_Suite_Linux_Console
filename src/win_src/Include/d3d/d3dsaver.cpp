...
HRESULT CD3DScreensaver::BuildDeviceList()
{
    ...
    for( UINT iAdapter = 0; iAdapter < m_pD3D->GetAdapterCount(); iAdapter++ )
    {
        ...
        D3DFORMAT      AllowedAdapterFormatArray[] = 
        ...
        {
            AdapterFormat = AllowedAdapterFormatArray[iaaf];
            dwNumAdapterModes = m_pD3D->GetAdapterModeCount( iAdapter, AdapterFormat );
            for( UINT iMode = 0; iMode < dwNumAdapterModes; iMode++ )
            {
                // Get the display mode attributes
                D3DDISPLAYMODE DisplayMode;
                m_pD3D->EnumAdapterModes( iAdapter, AdapterFormat, iMode, &DisplayMode );
